...
class XData():
    def __init__(self, start=0, end=2*np.pi, start_test=0.05, step=0.1, noise=False, seed=42) -> None:
        ...
        self.x_test = (np.array(x_test)).reshape(len(x_test), 1)

        if noise:
            noise = np.random.normal(0, 0.1, (len(x),1))
            self.x += noise
            noise_test = np.random.normal(0, 0.1, (len(x),1))
...
class SinusData(XData):
    def __init__(self, start=0, end=2*np.pi, start_test=0.05, step=0.1, noise=False, seed=42) -> None:
        ...
        self.y_test = self.y_test.reshape(len(self.x_test), 1)


class SquareData(XData):
    def __init__(self, start=0, end=2*np.pi, start_test=0.05, step=0.1, noise=False, seed=42) -> None:
        super().__init__(start, end, start_test, step, noise, seed)
        self.y = np.array([0 if sin_val < 0 else 1 for sin_val in np.sin(
            2*self.x)]).reshape(len(self.x), 1)
        self.y = self.y.reshape(len(self.x), 1)
        self.y_test = np.array([0 if sin_val < 0 else 1 for sin_val in np.sin(
            2*self.x_test)])
        self.y_test = self.y_test.reshape(len(self.x_test), 1)
